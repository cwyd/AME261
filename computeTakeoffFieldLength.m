...
groundEffectFactor = (16*wingDistAboveGround / b) / (1 + (16 * wingDistAboveGround/b)^2);

AR = b^2 / S;

CD = CD0 + groundEffectFactor * CLMax^2 / (pi * ev * AR); 

PRTakeoff = real((1 / (CLMax^(3/2) / CD)) * sqrt(2 * W0^3 / (rho * S)));

Vstall = sqrt(2 * W0 / (rho * S * CLMax));

VLO = 1.2 * Vstall; 

gammaClimb = real(asin(PATakeoffOneEngineOut - PRTakeoff / (W0 * VLO)));
...
balancedFieldLength = (0.863 / (1 + 2.3 * GFactor)) * ((W0/S)/ (rho * g) + hObstacleFeet)...
    * (1 / (Taverage / W0 - UFactor) + 2.7) + (655 / sqrt(rho / rhoSL));
